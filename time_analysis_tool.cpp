...
{
	char shell_str[256];
	strcpy(shell_str , "mkdir -p ./simulator_result/procedure_time");
	int ret = system(shell_str);
	assert(0 == ret);
	
	FILE *fp = fopen("./simulator_result/procedure_time/time_analysis.log","w");
	double total_time = 0;
	...
	{
		struct time_struct *ts = it->second;
		//printf("%s : %.6f ms \t\t\t total_time : %.6f ms ( cnt : % llu )\n",(it->first).c_str(), ts->latency / ts->cnt, ts->latency , ts->cnt);
