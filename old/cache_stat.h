...
double* re_retio;
long start_time;
uint64_t start_total_seq;
uint64_t start_re_seq;
*/
struct SeqNum
{
	/*y = alog(x) + b*/
...
long current_day;
long start_time;
unordered_map<string,unordered_map<uint64_t, struct BlockStat *> > stat_map;
...
private:
	...
	unordered_map<int, unordered_map<uint64_t, struct BlockStat*> > stat_map;
	unordered_map<long, struct SeqNum *> seq_map;
	double a;
	double b;
	int smcd_id;
...
public :
	...
	~StatCache();
	void main_operation(IoRecord* ir, long block_id, bool get_reuse_dis);
