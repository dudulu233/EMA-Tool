...
void StatCache::main_operation(IoRecord* ir, long block_id, bool get_reuse_dis)
{
	...
	auto index_it = stat_map.find(ir->disk_id);
	struct BlockStat *bs;
	int reuse_dis = -1;
	...
	//第一层索引，disk_sn(disk_id)
	if(index_it == stat_map.end())
	{
		//创建新的block_id的记录
		unordered_map<uint64_t,struct BlockStat *> maptmp;
		bs = (struct BlockStat *)malloc(sizeof(struct BlockStat));
		bs->last_time = ir->alloc_time;
		bs->last_total_seq = total_seq;
		maptmp[block_id] = bs;

		//将新的block_id的记录加入至disk_sn(disk_id)的map中
		stat_map[ir->disk_id] = maptmp;
		++ map_size;
		reuse_dis = 1023;
	}
	else
	{
		unordered_map<uint64_t , struct BlockStat *> *maptmp = &(index_it->second);
		auto it = maptmp->find(block_id);
		//第二层索引，block_id
		if(it == maptmp->end())
		{
			bs = (struct BlockStat *)malloc(sizeof(struct BlockStat));
		...
		else
		{
			...
			double reuse_tmp = (total_seq - bs->last_total_seq) *
									(1 - re_ratio);
			reuse_dis = (int)( reuse_tmp * 32 / 1024/ 1024 );
			if(reuse_dis >= 1024)
			{
